...
Feature wishlist:
- Double checks for paramters at startup/before rendering (ex: camera view specified no longer exists)
- Render start/end scripts
- Change frame range override to string
- Be able to handle camera name changes
"""

APP_NAME = 'Alternate Batch Render'
APP_VERSION = '0.1.0'  # Semantic Versioning https://semver.org/
AUTHOR = 'Logan Carrozza'
...
class BatchRenderDialog(QDialog):
    def __init__(self, parent=None):
        ...
        for camera in rt.cameras:
            try:
                ...
                continue
            self.cmbCamera.addItem(camera.name, camera)
    ...
    def start_batch_render(self):
        ...
        for row in range(total_rows):
            ...
            if item_values['Use']:
                ...
                camera_name = item_values['Camera']
                output_path = item_values['Output Path']
                frame_range = item_values['Range']
                resolution = item_values['Resolution']
                pixel_aspect = item_values['Pixel Aspect']
                scene_state_disp = item_values['Scene State']
                preset_disp = item_values['Render Preset']
                layer_preset_disp = item_values['Layer Preset']
                ...
                for x, frame in enumerate(frame_range):
                    ...
                    outputfile = os.path.join(directory, f"{filename}{frame:04d}{file_extension}")
                    print(f"Rendering frame {frame} ({x + 1}/{total_frames})")

                    debug_disable_render = False
                    if not debug_disable_render:  # Set false for debug if needed
    ...
    def camera_changed(self):
        ...
        if not self.system_modified:
            self.system_modified = True
            value = self.cmbCamera.currentText()

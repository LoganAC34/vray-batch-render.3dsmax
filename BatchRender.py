...
def setup_log():
    ...
    pipe_name = rf"\\.\pipe\{APP_NAME}_{os.getpid()}"
    global pipe_path
    ...
    secondary_window_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "secondary_window.py")
    )
    creation_flags = subprocess.CREATE_NO_WINDOW if not SHOW_LOG_LOG else False
    subprocess.Popen(
        ["python", f'"{secondary_window_path}"', f"{pipe_name}"], creationflags=creation_flags
    )
...
def send_to_log(data: dict):
    ...
    Currently only supports a single data type: JSON encoded dictionaries with
    'TYPE' and 'VALUE' keys.
    TYPE can be 'COMMAND' or 'LOG'.
    VALUE can be any string.
    If TYPE is 'COMMAND' and VALUE is 'SHUTDOWN', the named pipe is closed after
    writing the data.
    ...
    data_value = (
        data["VALUE"]
        if data_type != "LOG"
        else (LOG_INDENT_SPACING * LOG_INDENT_LEVEL) + data["VALUE"]
    )
...
def adjust_log_indent_level(adjustment: int):
    """
    Sets log output messages indent level. Useful for subtask output to be nested in previous log entries.
...
def convert_path_to_relative(absolute_path: str) -> str:
...
class BatchRenderDialog(qt_widgets.QDialog):
    ...
    def _build_gui(self):
        ...
        self.renderOutput = convert_path_to_relative(rt.GetDir(rt.Name("renderoutput")))
        ...
        # Table
        self.naming_tooltip = (
            "Valid replacement flags (not case sensitive):\n"
        ...
        self.resizable_columns = range(
            1, len(self.column_names) + 1
        ...
        self.tableWidget.setSelectionBehavior(
            qt_widgets.QAbstractItemView.SelectionBehavior(1)
        )
        ...
        self.groupBoxSelectedParams = qt_widgets.QGroupBox(
        ...
        self.lblSceneState = qt_widgets.QLabel(
        ...
        groupBoxLayout1.addItem(
            qt_widgets.QSpacerItem(1, 1, qt_widgets.QSizePolicy.Policy.Expanding), 1, 4
        )
        ...
        groupBoxLayout3.addItem(
            qt_widgets.QSpacerItem(1, 1, qt_widgets.QSizePolicy.Policy.Expanding), 2, 3
    ...
    def saveDialogData(self):
        ...
        key = winreg.OpenKey(
            winreg.HKEY_CURRENT_USER, f"Software\\{PUBLISHER}", 0, winreg.KEY_WRITE
        )
        ...
        if selected_rows:
            ...
            for row_int in reversed(rows_to_duplicate):
                ...
                for col in range(self.tableWidget.columnCount()):
                    ...
                    log(
                        "INFO",
                        f"{LOG_INDENT_SPACING}{column_name}: {value} [{value_type}] ({hidden_value})",
                    )
            ...
            rows_to_deselect = qt_widgets.QTableWidgetSelectionRange(
            ...
            rows_to_select = qt_widgets.QTableWidgetSelectionRange(
                start_row, start_col, end_row, end_col
            )
    ...
    def move_row(self, from_row: int, to_row: int):
        ...
        for col, column_name in enumerate(self.column_names):
            ...
            if isinstance(value, bool):  # Checkbox
                ...
                self.tableWidget.setCellData(
                    to_row, col, new_item, items_dict[column_name + "_Hidden"]
                )
    ...
    def start_batch_render(self, pre_check: bool = False):
                ...
                for line in file:
                    ...
                    if line_timestamp_datetime >= time_of_render and "error: " in line:
                        error_message = line.replace(
                            line_timestamp_formatted + " error", "Error"
                        ).strip()
        ...
        def get_frame_range(frame_range):
            if frame_range == "Default":
                ...
                ):  # Check if frame range is single frame "EX: 1:1"
